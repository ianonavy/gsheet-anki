"""
Creates Anki decks for studying for the EA Part 1 exam
"""
...
def create_anki_decks(spreadsheet_url, service_account_credentials):
...
def main():
    ...
    decks = create_anki_decks(spreadsheet_url, service_account_credentials)
    for deck in decks:
        deck_name = deck.name
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file = f'{deck_name.replace(" ", "_").lower()}_{timestamp}.apkg'
        export_deck_to_file(deck, output_file)
